...
- **不使用 DHT 发现设备列表**（DHT 设备数量过多，且无法预知连接质量）
- 通过**分享码**在 DHT 查找特定设备的 PeerId
- 建立连接后才显示网络状态（中继/打洞）和延迟
- NAT 穿透流程：
  1. 输入分享码 → DHT 查找 PeerId
  2. 尝试直连（如果双方都是公网）
  3. 尝试 DCUtR 打洞（NAT 穿透）
  4. 打洞失败则使用 Relay 中继
- 连接引导节点加入 DHT 网络
...
| 类型 | 发现方式 | 延迟 | 说明 |
|------|---------|------|------|
| 局域网 | mDNS | ~2ms | 本地网络直连，速度最快 |
| 打洞 | 分享码 + DHT | 10-100ms | DCUtR NAT 穿透后直连 |
| 中继 | 分享码 + DHT | 100-500ms | 通过 Relay 服务器转发 |
...
> **MVP 阶段**：仅在内存中保留当前会话的传输记录，应用重启后清空

- 发送/接收历史列表
- 显示：时间、对方设备名、文件列表、状态
- 可清除历史记录
- **可选**：SQLite 持久化 + 隐私开关（默认关闭）
...
> **MVP 阶段**：仅实现分块传输 + 单块重试（3 次），网络闪断 30 秒内自动重连
